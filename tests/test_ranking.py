...
CATALOG = providers_for_category(PORT_CONGESTION)
SATELLITE_META = get_provider("satellite-logistics-intel")
TELEMETRY_META = get_provider("terminal-ops-telemetry")
...
def rank(**overrides):
    ...
    return ranking.rank_providers(CATALOG, **kwargs)
...
def test_best_candidate_is_the_most_confidence_per_drop():
    ...
    best = rank().best
    assert best.provider_id == "satellite-logistics-intel"
...
def test_already_covered_evidence_has_no_marginal_value():
    ...
    satellite = next(
        c for c in result.candidates if c.provider_id == "satellite-logistics-intel"
    )
...
def test_purchased_providers_are_excluded():
    result = rank(already_purchased={"satellite-logistics-intel"})
    satellite = next(
        c for c in result.candidates if c.provider_id == "satellite-logistics-intel"
    )
    assert not satellite.eligible
    assert "Already purchased" in satellite.reason
    assert result.best.provider_id == "terminal-ops-telemetry"
...
def test_ranking_is_serialisable_for_the_ui():
    ...
    assert payload[0]["provider_id"] == "satellite-logistics-intel"
